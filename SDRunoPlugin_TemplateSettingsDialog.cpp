...
void SDRunoPlugin_TemplateSettingsDialog::BuildUi()
{
    ...
    // Configure title
    m_title.caption("VRX disponibles");
    ...
    m_btnRefresh.events().click([this]() { 
        PopulateVrxList(); 
    });
    ...
    m_btnClose.events().click([this]() { 
        this->close(); 
    });

    // Double click: choose VRX
...
#else
    // On other platforms, no-op for now
#endif
}
...
void SDRunoPlugin_TemplateSettingsDialog::PopulateVrxList()
{
    ...
    int count = 0;
    try { 
        count = m_controller.GetVRXCount(); 
    } catch (...) { 
        count = 0; 
    }

    if (m_vrxList.size_categ() == 0) {
        m_vrxList.append("VRX");
    }
    ...
    for (int i = 0; i < count; ++i) {
        bool enabled = false;
        try { 
            enabled = m_controller.GetVRXEnable(i); 
        } catch (...) { 
            enabled = false; 
        }
