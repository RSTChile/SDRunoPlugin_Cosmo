    ...
    : IUnoPlugin(controller)
{
    ...
    // Patrón probado en los plugins de JvanKatwijk:
    // 1) Forzar IQOUT en VRX 0
    // 2) Registrar AudioProcessor en VRX 0
    m_vrxIndex = 0;
    try {
        m_controller.SetDemodulatorType(m_vrxIndex, IUnoPluginController::DemodulatorIQOUT);
    } catch (...) {}
    try {
        m_controller.RegisterAudioProcessor(m_vrxIndex, this);
    } catch (...) {}
}
...
// Núcleo: capturamos IQ desde AudioProcessorProcess con IQOUT (192 kS/s)
// buffer: float intercalado, PERO SDRuno invierte I y Q -> I=buffer[2*i+1], Q=buffer[2*i]
void SDRunoPlugin_Template::AudioProcessorProcess(channel_t channel, float* buffer, int length, bool& modified) {
    (void)modified; // no modificamos el stream de audio
    try {
        // Si nos piden descargar, háganse en hilo del host
        ...
        // Adoptar canal reportado (sólo informativo por ahora, trabajamos con VRX 0)
        if (static_cast<int>(channel) != m_vrxIndex) {
            m_vrxIndex = static_cast<int>(channel);
        }

        // Marcamos streaming activo al recibir el primer buffer
        if (!m_isStreaming.exchange(true)) {
            if (m_ui) m_ui->SetStreamingState(true);
        }

        // Aplicar cambios pendientes (carpeta/modo/vrx)
        ApplyPendingBaseDirIfAny();
        ApplyPendingModeIfAny();
        ApplyPendingVrxIfAny(); // futuro: re-registrar audio processor en otro VRX

        // Copiar a vector IQ (I,Q por muestra)
        std::vector<float> iq;
        ...
        // Captura a disco si está habilitado
